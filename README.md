...
Grab the patches from inside the /patches/ folder from the GitHub page, or alternatively, download the .zip file from the Releases page (once a proper release is out) and apply the patch over your Metroid Zero Mission ROM with [Floating IPS (FLIPS)](https://git.disroot.org/Sir_Walrus/Flips).
...
* **SpineShark** - Zero Charge hack fix for Samus not shooting a charged pistol shot when crouching. Fix for Full Suit not showing on post-Charlie activation sequence. Starting Items with Fanfare code.
